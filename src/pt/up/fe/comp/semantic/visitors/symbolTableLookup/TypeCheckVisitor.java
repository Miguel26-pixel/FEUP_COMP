...
public class TypeCheckVisitor extends ReportCollectorJmmNodeVisitor<Type, Type> {
    ...
    private Type visitIntLiteral(JmmNode node, Type dummy) {
        return new Type("int", false);
    }

    private Type visitBooleanLiteral(JmmNode node, Type dummy) {
        return new Type("boolean", false);
    }

    private Type visitThisLiteral(JmmNode node, Type type) {
        if (node.getAncestor("MainMethod").isPresent()) {
            addSemanticErrorReport(node, "Cannot reference this on static methods");
        }
    ...
    private Type visitIdentifier(JmmNode node, Type dummy) {
        if (node.getAncestor("MainMethod").isPresent()
                && symbolTable.getLocalVariables("main").stream().noneMatch(s -> s.getName().equals(node.get("name")))
                && symbolTable.getFields().stream().anyMatch(s -> s.getName().equals(node.get("name")))) {
            addSemanticErrorReport(node, "Cannot access class fields from static methods");
            return new Type("", false);
        }
